...
class VerifyOTPLoginScreen extends StatefulWidget {
  const VerifyOTPLoginScreen({super.key, this.phoneNumber, this.email});
  final String? phoneNumber;
  final String? email;

  @override
  State<VerifyOTPLoginScreen> createState() => _VerifyOTPLoginScreenState();
}

class _VerifyOTPLoginScreenState extends State<VerifyOTPLoginScreen> {
  ...
  Future<void> _verifyOtp() async {
    try {
      ...
      AuthResponse response = widget.email == null
      ...
      analytics.logLogin(
          loginMethod: widget.email == null ? "Reset phone" : "Reset email",
          parameters: {"confirmed": "true"});
      debugPrint((response.user == null).toString());
    } on AuthException catch (error) {
      if (error.statusCode == '400') {
        _loginErrorDialog(
          title: "Problème de connexion",
          content: widget.email == null
              ? "Vérifiez votre code ou votre numéro de téléphone."
              : "Vérifiez votre email ou votre code.",
        );
      ...
      if (widget.phoneNumber != null) {
        try {
          ...
          if (res.data["success"]) {
            ...
            analytics.logEvent(
                name: "Resend OTP",
                parameters: {"type": widget.email == null ? "Phone" : "Email"});
